...
class Lobby {
  ...
  hasEmptySeat() {
    return this.peers.length < this.seats;
  }
  ...
  join(peer) {
    ...
    if (this.peers.length === this.seats) {
      this.seal(this.host);
    }
  }
  ...
  seal(peerId) {
    ...
    console.log(
      `Peer ${peerId} sealed lobby ${this.name} ` +
...
function createNewLobby(peer, mesh) {
...
function joinNewLobby(peer, mesh) {
  const lobby = createNewLobby(peer, mesh);
  joinLobby(peer, lobby.name);
}

function joinFirstAvailableLobby(peer, mesh) {
  const availableLobby = getAvailableLobby();
  if (availableLobby) {
    joinLobby(peer, availableLobby.name);
    return;
  }
  joinNewLobby(peer, mesh);
}

function getAvailableLobby() {
  for (const [name, lobby] of lobbies) {
    if (!lobby.sealed && lobby.hasEmptySeat()) {
      return lobby;
    }
  }
  return null;
...
function parseMsg(peer, msg) {
  ...
  if (type === CMD.JOIN) {
    if (data === "") {
      joinFirstAvailableLobby(peer, false);
      return;
    }
    joinLobby(peer, data);
    return;
  ...
  ws.on("message", (message) => {
    try {
      ...
      if (typeof data !== "string") {
        ...
        ws.close(
          4000,
          `An error occured while parsing the message: ${e.message}`
        );
        return;
      }
    }
